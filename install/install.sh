...
preflight_checks() {
  ...
  if [ ! -d "$src_path" ]; then
    _preflight_fail \
      ...
      "clone the repository via 'git clone https://github.com/cartagena/InkyPi.git' and run install/install.sh from inside the clone"
  fi
  if ! git -c safe.directory='*' -C "$src_path" rev-parse --git-dir >/dev/null 2>&1; then
    _preflight_fail \
      "source tree at $src_path is not a git repository" \
      "install.sh must run from a git clone (not a downloaded tarball) so 'git describe' and waveshare pin verification work. Run: git clone https://github.com/cartagena/InkyPi.git"
  fi
...
ask_for_reboot() {
  ...
  echo_header "[•] If you encounter any issues or have suggestions, please submit them here: https://github.com/cartagena/InkyPi/issues"
