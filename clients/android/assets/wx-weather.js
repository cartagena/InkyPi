...
(function () {
  ...
  /* The panel's two drawn sections live in wx-weather-panel.js — moved there for the
     500-line budget. Read at parse time, so that file precedes this one in index.html. */
  var sunArc = WP.wxPanel.sunArc, rainSection = WP.wxPanel.rainSection;
  ...
  var weather = {
    ...
    init: function () {
      /* WP.place(), not CONFIG: a town chosen on the tablet wins over the one the build was
         made with, and choosing one has to work on a panel that booted with no location at
         all — hence the listener above the early return rather than below it. */
      var self = this;
      WP.onPlaceChange(function () { self.onPlaceChanged(); });
      var loc = WP.place();
      if (loc.latitude == null || loc.longitude == null) {
        $("wx-temp").textContent = "--°";
        $("wx-desc").textContent = "No location set";
        $("wx-meta").textContent = "Open Setup and search for your town";
        WP.status("Weather is off until a location is set", true);
        return;
      }
    ...
    /* A new town is a new everything: the cached payload is the old place's weather, so it
       goes rather than sit on screen under the new name. */
    onPlaceChanged: function () {
      this.data = null;
      this.stale = false;
      this.fetchedAt = 0;
      WP.store.write(WX_CACHE, "");
      this.fetch();
    },
    ...
    fetch: function () {
      var self = this;
      if (WP.place().latitude == null) return;
    ...
    render: function () {
      ...
      var i = this.nowIndex();
      var bits = [];
      if (WP.place().name) bits.push(WP.place().name);
    ...
    /* One line, from state, in a person's words. It used to read "<place> · live" /
       ...
       (The place name itself is never written into a source file: it is whatever the
       panel's chosen location is called at runtime.) */
    ...
    panelSub: function () {
      ...
      var where = WP.place().name ? WP.place().name + " · " : "";
